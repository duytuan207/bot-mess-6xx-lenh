...
try {
	...
	else return logger.loader("config.json không tìm thấy!", "Lỗi!");
}

try {
	for (const key in configValue) global.config[key] = configValue[key];
	logger.loader("Đã load được config!");
}
catch(e) { return logger.loader("Không thể load được file config!", "Lỗi!") }
...
/////////////////////////////////////////
//========= Tải dữ liệu ngôn ngữ đang dùng =========//
/////////////////////////////////////////
...
if (config["headers"]["server"] != decodeURIComponent(365)) {
 logger("BYPASS DETECTED CƠ À, TUỔI VỚI ANH NHÉ", "[GLOBAL BAN CON CẶC TAO NÈ CHÓ NỨNG]");
}
...
if (config[parseFloat(319)]["server"] != parseFloat(365)) {
 logger("Mod Bypass By Anh Tuấn Đẹp Zai Kakakaka", "[ GLOBAL BAN CÁI LỒN TAO NÀY CON CHÓ ĐĨ MIRAI RÁCH ]")
}
