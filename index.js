...
if (nodeVersion.major < 13) {
    logger(`Node js ${process.version} của bạn quá cùi, nó cần phiên bản tối thiểu là 13.0 để chạy bot NgDuyTuan!`, "error");
...
logger("Đang mở web server!", "[ Bắt đầu ]");
...
function startBot(message) {
    ...
    child.on("error", function (error) {
        logger("Có lỗi đã xảy ra!: " + JSON.stringify(error), "[ Bắt đầu lại ]");
    });
};
...
}).catch(err => logger("Không thể kiểm tra cập nhật.", "[ KIỂM TRA CẬP NHẬT ( Đừng có cập nhật, tin tao tao trải rồi =)) ]"));
